(() => {
  ...
  const planCopy = {
    ru: { planLabel: 'ВАШ ПЛАН', planTitle: 'Начните с простых шагов', planText: 'Отмечайте выполненное — прогресс сохранится на этом устройстве.', planProgress: 'Выполнено', planProfile: 'Настройте подборку', planProfileText: 'Выберите город и направление.', planExplore: 'Изучите возможности', planExploreText: 'Откройте вакансии, гранты или мероприятия.', planCommunity: 'Задайте вопрос сообществу', planCommunityText: 'Найдите опыт студентов с похожими целями.' },
    kz: { planLabel: 'СІЗДІҢ ЖОСПАРЫҢЫЗ', planTitle: 'Қарапайым қадамдардан бастаңыз', planText: 'Орындалғанын белгілеңіз — нәтиже осы құрылғыда сақталады.', planProgress: 'Орындалды', planProfile: 'Ұсыныстарды баптаңыз', planProfileText: 'Қала мен бағытты таңдаңыз.', planExplore: 'Мүмкіндіктерді зерттеңіз', planExploreText: 'Вакансияларды, гранттарды немесе іс-шараларды ашыңыз.', planCommunity: 'Қауымдастыққа сұрақ қойыңыз', planCommunityText: 'Ұқсас мақсаттары бар студенттердің тәжірибесін табыңыз.' },
  };
  ...
  const update = () => {
    ...
    document.querySelectorAll('[data-i18n]').forEach((node) => { const value = isKz ? (kz[node.dataset.i18n] || planCopy.kz[node.dataset.i18n]) : planCopy.ru[node.dataset.i18n]; if (value) node.innerHTML = value; });
